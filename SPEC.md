...
- Chat panel (right side, `c` toggles it): natural-language questions go to `POST CHAT_BASE/api/chat` with the conversation and the map context. The response's `ui` object becomes the assistant layer: safe route in the accent colour with the fast route faint, area ring, halos and markers for every relevant event, and up to 6 compact event cards (highest-risk highlighted events; clicking a card opens the full popup). A new answer replaces the layer; Reset view clears it and keeps the conversation; New chat clears both; "Show on map" re-applies an earlier answer. The assistant's route is independent of the Walking route panel's. `?demoChat` gives canned responses in sample mode.
